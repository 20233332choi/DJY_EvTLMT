...
TV=Path.home()/'Documents/TV'

class BoardTimeSyncTests(unittest.TestCase):
    def test_protocol_gating_cookies_and_sensor_frames(self):
        with tempfile.TemporaryDirectory() as tmp:
            folder=Path(tmp)
            shutil.copyfile(TV/'stm_back/Core/Src/board_time_sync.c',folder/'board_time_sync.c')
            ...
            result=subprocess.run(compiler()+['-std=gnu11','-O2','-Wall','-Werror','-I',str(folder),
                '-I',str(TV/'stm_back/Core/Inc'),str(source),'-o',str(exe),'-lm'],capture_output=True,timeout=180)
    ...
    def test_shared_sources_identical(self):
        for name in ('Inc/vehicle_clock.h','Inc/board_time_sync.h','Inc/time_sync_model.h',
            ...
            self.assertEqual((TV/'stm_back/Core'/name).read_bytes(),(TV/'stm_front/Core'/name).read_bytes(),name)
    ...
    def test_offset_drift_wrap_reboot_missing_and_delayed_frames(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result=subprocess.run(compiler()+['-std=gnu11','-O2','-Wall','-Werror','-I',str(TV/'stm_back/Core/Inc'),str(source),'-o',str(exe),'-lm'],capture_output=True,timeout=180)
            self.assertEqual(result.returncode,0,result.stderr.decode(errors='replace'))
            result=subprocess.run([str(exe)],capture_output=True,timeout=10)
            self.assertEqual(result.returncode,0,result.stderr.decode(errors='replace'))
