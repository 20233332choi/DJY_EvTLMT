...
static void  Debug_SendESP(void);
...
/* Read-only 5Hz ESP telemetry. PA10 never processes control commands.
 * The IT transmitter owns line until completion; skip busy/oversize samples.
 * ctl/req/lim/app/urx/uerr are zero: this firmware has no remote pit control.
 * TV/ED and DAC fields are the actual local controller state. */
static void Debug_SendESP(void) {
    static char line[768];
    if (huart1.gState != HAL_UART_STATE_READY || huart2.gState != HAL_UART_STATE_READY) return;
    uint32_t mask = __get_PRIMASK();
    __disable_irq();
    TV_t sample = tv;
    // STOP clears TV_t for control logging, but the DAC still holds its off code.
    if (Safety_GetAction() == SAFE_ACTION_STOP) {
        sample.dac_left = sample.dac_right = (uint16_t)(V_THROTTLE_OFF * DAC_CODE_PER_V);
    }
    SensorData_t s = CAN_GetSensorData();
    unsigned rpm_l = RPM_GetLeft(), rpm_r = RPM_GetRight();
    unsigned fresh_l = RPM_IsLeftFresh(), fresh_r = RPM_IsRightFresh();
    unsigned imu_valid = IMU_IsTelemetryFresh();
    float yaw = IMU_GetYawRate(), lat = IMU_GetLateralAcc();
    float ax = IMU_GetRawAx(), ay = IMU_GetRawAy(), az = IMU_GetRawAz();
    unsigned fault = CAN_IsSensorFresh() ? (unsigned)Safety_GetFaultCode() : FAULT_CAN_TIMEOUT;
    unsigned sas_valid = CAN_IsSensorFresh() && CAN_IsHeartbeatFresh() &&
                         !(CAN_GetHeartbeatStatus() & HB_STATUS_SAS_ERR);
    uint32_t cap_l = g_rpm_cap_count_l, cap_r = g_rpm_cap_count_r;
    uint32_t glt_l = g_rpm_glitch_l, glt_r = g_rpm_glitch_r;
    __set_PRIMASK(mask);
    int n = snprintf(line, sizeof(line),
        "L=%u R=%u cap=%lu/%lu glt=%lu/%lu dsy=%lu/%lu tps=%u pct=%d idle=%u "
        "imu=%u sas=%u yaw=%ld lat=%ld lon=%ld ax=%ld ay=%ld az=%ld "
        "vs=%ld dy=%ld ye=%ld dp=%ld pl=%ld pr=%ld tva=%u eda=%u tr=%ld "
        "ctl=0/0 req=0 lim=0 app=0 tv=%u ed=%u fault=%u dac=%u/%u "
        "sas=%u imu=%u urx=0 uerr=0 rv=%u/%u steer=%ld sv=%u sc=%u "
        "can=%lu/%lu/%08lx idg=%lu/%lu/%lu/%lu kp=%ld ki=%ld kd=%ld\r\n",
        rpm_l, rpm_r, (unsigned long)cap_l, (unsigned long)cap_r,
        (unsigned long)glt_l, (unsigned long)glt_r,
        (unsigned long)g_rpm_desync_l, (unsigned long)g_rpm_desync_r,
        (unsigned)s.tps_raw, (int)(TPS_to_Fraction((float)s.tps_raw) * 100.0f),
        (unsigned)s_tps_idle, imu_valid, (unsigned)s.sas_angle,
        (long)(yaw * 1000.0f), (long)(lat * 1000.0f), (long)(ax * 1000.0f),
        (long)(ax * 1000.0f), (long)(ay * 1000.0f), (long)(az * 1000.0f),
        (long)(sample.vehicle_speed * 1000.0f), (long)(sample.desired_yaw * 1000.0f),
        (long)(sample.yaw_error * 1000.0f), (long)(sample.delta_power * 1000.0f),
        (long)(sample.power_left * 1000.0f), (long)(sample.power_right * 1000.0f),
        (unsigned)sample.tv_active, (unsigned)sample.ed_active,
        (long)(sample.traction_scale * 1000.0f),
        (unsigned)sample.tv_active, (unsigned)sample.ed_active, fault,
        (unsigned)sample.dac_left, (unsigned)sample.dac_right,
        (unsigned)s.sas_angle, imu_valid, fresh_l, fresh_r,
        (long)(SAS_to_SteeringAngle((float)s.sas_angle) * 1000.0f), sas_valid, (unsigned)SAS_CENTER_RAW,
        ...
        /* These are the gains passed to TV_Init; this firmware has no gain setter. */
        (long)(PID_KP * 1000.0f), (long)(PID_KI * 1000.0f), (long)(PID_KD * 1000.0f));
    if (n > 0 && n < (int)sizeof(line)) {
        (void)HAL_UART_Transmit_IT(&huart1, (uint8_t *)line, (uint16_t)n);
        (void)HAL_UART_Transmit_IT(&huart2, (uint8_t *)line, (uint16_t)n);
    }
}
