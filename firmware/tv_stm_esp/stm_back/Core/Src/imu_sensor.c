...
static float s_raw_ax = 0.0f, s_raw_az = 0.0f;
...
static uint16_t dma_write_idx(void) {
    return (uint16_t)(RX_BUF_SIZE - __HAL_DMA_GET_COUNTER(&hdma_uart4_rx));
}
...
static bool parse_packet(const uint8_t *p) {
    ...
    if (p[1] == WT_GYRO) {
        ...
        s_raw_ax = (float)(int16_t)((uint16_t)p[2] | ((uint16_t)p[3] << 8)) / 32768.0f * 16.0f * G_ACC;
        s_raw_az = (float)(int16_t)((uint16_t)p[6] | ((uint16_t)p[7] << 8)) / 32768.0f * 16.0f * G_ACC;
        s_acc_ms = HAL_GetTick();
        g_imu_acc_ok++;
        if (s_cal_active) {          /* 캘리브레이션 중이면 원본값을 누적 */
...
float IMU_GetRawAy(void) { return s_lat_acc; }
float IMU_GetRawAz(void) { return s_raw_az; }
bool IMU_IsTelemetryFresh(void) {
    return g_imu_gyro_ok != 0 && g_imu_acc_ok != 0 && IMU_IsValid() &&
           (HAL_GetTick() - s_acc_ms) <= IMU_TIMEOUT_MS;
}
