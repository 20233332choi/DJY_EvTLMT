#pragma once

/* Official self-signed Let's Encrypt Generation Y trust anchors.
 * ngrok currently serves an ECDSA chain through Root YE. Root YR is included
 * so a future RSA certificate rotation does not silently break telemetry. */
static constexpr char kLetsEncryptGenYRoots[] = R"CERT(-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----)CERT";
