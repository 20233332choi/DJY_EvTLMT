...
const groups = {yaw:'01 · Yaw 제어 / 목표·실제·STM 오차',power:'02 · 출력 배분 / 좌·우·차동전력',rpm:'03 · 모터 RPM / 측정·보고·목표',speed:'04 · 차량 속도',steering:'05 · 조향 입력',throttle:'06 · 스로틀 입력',traction:'07 · 트랙션 개입 배율',activity:'08 · TV / ED 개입 상태',voltage:'09 · 모터 전압'};
const groupTitle = group => groups[group] || (group==='settings'?'PID 보드 보고값':'STM 차속 원본');
...
function init(channels) {
  ...
  let saved=null;try{saved=JSON.parse(localStorage.getItem('ev-tuning-channels-v2'));}catch{}
  state.enabled=new Set(Array.isArray(saved)?saved:channels.map(c=>c.key));
  const containers={};
  for(const group of [...Object.keys(groups),'speed_raw','settings']){
  ...
  channels.forEach(c=>{
    ...
    input.onchange=()=>{input.checked?state.enabled.add(c.key):state.enabled.delete(c.key);try{localStorage.setItem('ev-tuning-channels-v2',JSON.stringify([...state.enabled]));}catch{}draw();};
...
function drawPlot(group,view){
  ...
  for(const c of channels){
    ...
    for(const r of view.rows){const v=r.values[c.key];if(!finite(v)){prev=null;continue;}if(prev===null||r.time_ms-prev.time_ms>1500)ctx.moveTo(x(r.time_ms),y(v));else{if(group==='activity')ctx.lineTo(x(r.time_ms),y(prev.values[c.key]));ctx.lineTo(x(r.time_ms),y(v));}prev=r;}ctx.stroke();ctx.setLineDash([]);
...
async function sessions(){const data=await json('/api/tuning/sessions');const selected=$('session').value;$('session').replaceChildren(new Option('과거 측정 선택',''));for(const s of data.sessions)$('session').add(new Option(`#${s.id} · ${s.label||'측정'} · ${new Date(s.started_at_utc).toLocaleString('ko-KR')} · ${s.sample_count}개`,s.id));$('session').value=selected;}
async function history(page=0){
  const generation=++state.generation;state.busyHistory=true;$('prev').disabled=$('next').disabled=true;
  try{const data=await json(`/api/tuning/samples?session_id=${state.session}&after=${state.pages[page]||0}&through=${state.through}`);if(generation!==state.generation)return;state.history=data.samples;state.through=data.through;state.page=page;state.pages[page+1]=data.cursor;state.more=data.more;$('scrub').value=1000;$('mode').textContent=`과거 세션 #${state.session} · ${page+1}페이지 · ${data.samples.length}개`;$('error').textContent='';draw();}
...
async function poll(){
  let catchUp=false;
  try{
    const data=await json('/api/tuning/live?after='+state.cursor);
    ...
    state.recording=data.recording_active;$('recordStatus').textContent=data.recording_active?`● 기록 중 · 세션 #${data.recording_session_id} · ${data.recording_sample_count}개`:'기록 대기 · 측정 시작부터 DB에 저장';
    $('record').textContent=data.recording_active?'측정 종료':'측정 시작';$('record').disabled=!data.can_record;$('label').disabled=data.recording_active||!data.can_record;$('rawLink').hidden=!data.can_record;
...
$('record').onclick=async()=>{const active=state.recording;$('record').disabled=true;try{await json('/api/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:active?'recording_stop':'recording_start',label:$('label').value})});await sessions();$('error').textContent='';}catch(e){$('error').textContent=e.message;}};
...
$('export').onclick=()=>{const rows=state.mode==='live'?state.live.filter(r=>r.id):state.history;const keys=state.channels.map(c=>c.key);const csv=['captured_at_utc,source_timestamp_ms,'+[...keys,...rpmStatusKeys].join(','),...rows.map(r=>[new Date(r.time_ms).toISOString(),r.source_timestamp_ms??'',...keys.map(k=>finite(r.values[k])?r.values[k]:''),...rpmStatusKeys.map(k=>r.status?.[k]??'')].join(','))].join('\r\n');const url=URL.createObjectURL(new Blob(['\uFEFF'+csv],{type:'text/csv;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download=`ev-${state.mode}-${state.session||'live'}-page-${state.page+1}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
$('share').onclick=async()=>{try{await navigator.clipboard.writeText(location.origin+'/pit');$('notice').textContent='주소 복사됨. 다른 기기에는 localhost 대신 ngrok 주소를 공유하세요.';}catch{$('notice').textContent=location.origin+'/pit';}};
...
async function overview(){try{const t=await json('/api/tuning/overview');$('vehicle').textContent=`STM ${t.stm_online?'수신':'미수신'} · IMU ${t.imu_online?'수신':'미수신'} · TV ${t.tqv_internal_online?(t.tv_active?'개입':'대기'):'미수신'} · 속도 ${t.speed_online?format(t.speed_kmh,1)+' km/h':'미수신'}`;
  ...
  if(t.gnss_online&&finite(t.gnss_latitude_deg)&&finite(t.gnss_longitude_deg)){const fix=`${t.gnss_latitude_deg},${t.gnss_longitude_deg}`;if(fix!==state.lastFix){state.track.push([t.gnss_longitude_deg,t.gnss_latitude_deg]);state.track=state.track.slice(-5000);state.lastFix=fix;}}
  const {ctx,w,h}=surface($('track'));if(state.track.length){const lat=state.track[0][1],points=state.track.map(p=>[p[0]*Math.cos(lat*Math.PI/180),p[1]]),xs=points.map(p=>p[0]),ys=points.map(p=>p[1]),xmin=Math.min(...xs),xmax=Math.max(...xs),ymin=Math.min(...ys),ymax=Math.max(...ys),scale=Math.min((w-30)/Math.max(1e-6,xmax-xmin),(h-30)/Math.max(1e-6,ymax-ymin));const xy=p=>[w/2+(p[0]-(xmin+xmax)/2)*scale,h/2-(p[1]-(ymin+ymax)/2)*scale];ctx.strokeStyle='#00ffff';ctx.lineWidth=2;ctx.beginPath();points.forEach((p,i)=>{const [x,y]=xy(p);i?ctx.lineTo(x,y):ctx.moveTo(x,y);});ctx.stroke();ctx.fillStyle='#ffff00';ctx.beginPath();ctx.arc(...xy(points.at(-1)),4,0,Math.PI*2);ctx.fill();}
...
sessions().catch(e=>$('error').textContent=e.message);poll();overview();
